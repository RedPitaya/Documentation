...
本示例展示如何编程使 Red Pitaya 生成自定义波形信号。电压和频率范围取决于 Red Pitaya 型号。

.. note::

    可以向 Red Pitaya 发送少于完整缓冲区（16384 个采样点）的数据，但频率会相应调整。例如，如果只发送 8192 个采样点并将频率指定为 10 kHz，Red Pitaya 将生成 20 kHz 的信号。
