...
本示例展示如何编程 Red Pitaya，在外部触发下生成模拟信号。Red Pitaya 会先等待外部源的触发，触发条件满足后立即开始生成所需信号。同样的概念也适用于外部触发下的连续信号生成。电压和频率范围取决于 Red Pitaya 型号。
...
- `下载示例 <https://downloads.redpitaya.com/downloads/Clients/labview/Generate%20signal%20on%20external%20trigger.vi>`_
