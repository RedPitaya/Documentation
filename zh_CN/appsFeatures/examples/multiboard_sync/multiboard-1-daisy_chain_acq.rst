...
本示例演示如何同步 Red Pitaya X-channel 系统（菊花链），以便在多个设备上同时采集所生成信号的 16k 个样本（快速 RF 输入和输出）。

所需硬件
===================

    - 主 Red Pitaya 设备（STEMlab 125-14 LN）
    - 一个或多个从设备（STEMlab 125-14 LN Secondary）
    - SATA 线缆
    - SMA 线缆和 SMA T 型连接器

接线示例：

-   将主设备的 OUT1 连接到主设备的 IN1 和从设备的 IN1。
-   使用 SATA 线缆连接主设备和从设备
...
.. code-block:: matlab
    ...
    % This example is for X-channel System connected with SATA cables
