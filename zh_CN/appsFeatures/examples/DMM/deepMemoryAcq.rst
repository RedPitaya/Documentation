...
本示例演示如何使用深度内存采集将数据捕获到两个 1024 字节的缓冲区中。DMA 可以与标准 SCPI 采集并行工作。
...
有关深度内存采集的更多信息，请参阅 :ref:`深度内存采集 <deepMemoryAcq>` 章节。
...
.. note::
  ...
  将数据直接返回到 NumPy 缓冲区的新函数（NumPy 选项卡）比之前的版本（普通选项卡）快得多，在较长数据序列中尤其明显。
  传入的 NumPy 缓冲区必须使用 ``dtype=np.float32`` （采集 Volt 数据时）或 ``dtype=np.int16`` （采集 RAW 数据时）。
