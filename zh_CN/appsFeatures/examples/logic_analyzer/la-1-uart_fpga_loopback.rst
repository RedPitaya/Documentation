...
本示例演示如何使用逻辑分析仪命令解码 UART 协议。Red Pitaya 在 UART 端口上生成 UART 消息，并在指定的数字输入上对其进行解码。采集的数据存储在 ``uart_data.bin`` 文件中，可用于进一步分析。
配置本示例时，请确保 UART API 设置与 UART 解码器设置相匹配。

运行示例前，请将 **UART TX (E2) 引脚连接到 DIO4_P (E1)**。UART RX 引脚无需连接，因为它不发送任何数据。
