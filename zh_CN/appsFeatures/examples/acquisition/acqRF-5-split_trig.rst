...
分离触发采集（STEMlab 125-14 4-Input）
##################################################

.. note::

    未来计划将此功能扩展到其他 Red Pitaya 型号。

描述
============

通常 Red Pitaya 的所有通道共用一个触发器，但在分离触发模式下每个通道都可以拥有自己的触发器。 本示例展示如何在分离触发模式下从全部 4 个快速模拟输入采集信号的 6000 个采样点。 由于每个通道都有自己的触发器，通道满足其触发条件时便开始采集。
The decimations and time scales of a buffer are specified in the :ref:`sample rate and decimation <s_rate_and_dec>`. 电压和频率范围取决于 Red Pitaya 型号。
...
.. note::
    ...
    此代码适用于 **2.05-37 或更高版本的 OS**。对于较旧的 OS 版本，请确认各命令的发布时间（每个在 2.00 或更高版本中引入的命令旁均附有说明）。
