...
本示例展示如何从快速模拟输入采集信号的 16k 个采样点。当满足内部触发条件时，系统会采集信号。采集信号的时间长度取决于缓冲区时间刻度，该刻度可通过抽取因子设置。缓冲区的抽取因子和时间刻度见 :ref:`采样率和抽取 <s_rate_and_dec>`。电压和频率范围取决于 Red Pitaya 型号。
...
在本节选择采集数据的格式和单位。格式可以是 ASCII 或 BIN，单位可以是 VOLTS 或 RAW。
...
采集数据的解码取决于所选格式和单位。应将与所选单位和格式对应的以下代码添加到上方代码块末尾。
