...
有关将电路连接到 Red Pitaya 板卡的说明请参阅 :ref:`此处 <impedance_connection>`。
...
.. note::
    ...
    此代码适用于 **2.05-37 或更高版本的 OS**。对于较旧的 OS 版本，请检查具体命令的发布版本（每个在 2.00 或更高版本中引入的命令都附有说明）。


电路
=======

本教程需要使用 *LCR 测量仪扩展模块* 或 *外部分流电阻*。
每种方法都有优缺点。下面简要介绍硬件设置方法。
...
LCR 测量仪扩展模块连接更简单，并能在以下分流电阻值之间自动切换：
...
有关 LCR 测量仪扩展模块的更多信息，请参阅我们的 :ref:`文档 <lcr_extension_module>`。
...
.. note::
    ...
    为尽量减小 Red Pitaya 输入阻抗对测量的影响，请按照上图重新配置跳线（连接两个输入端的中间两个引脚），以旁路输入电阻分压器。
    这样会将**输入电压范围降低到 +-0.5 V**，因此请确保输出电压设置不超过 +-0.5 V（**绝对最大值 0.75 V（幅度 + 偏置）**）。
...
--------------
    ...
    LCR 测量仪需要在 ``Makefile`` 中包含两个额外库（"-lrpapp_lcr -lrp-dsp"）。请检查是否已添加；如果没有，请将其添加到*第 7 行*。
