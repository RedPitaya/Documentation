...
本示例展示如何在高速模拟输入上采集 16k 个信号样本。信号将与生成信号同时采集。采集信号的时间长度取决于缓冲区时间尺度，该尺度可通过抽取因子设置。缓冲区的抽取因子和时间尺度见 :ref:`采样率与抽取 <s_rate_and_dec>`。电压和频率范围取决于 Red Pitaya 型号。
...
.. code-block:: python
    ...
    rp.sour_set(1, wave_form, ampl, freq, burst=True, ncyc=3)

    ##### Acqusition #####
    # Function for configuring Acquisition
    rp.acq_set(dec=1, trig_lvl=0, trig_delay=0)
    ...
    data = rp.acq_data(1, convert= True)
...
- `下载示例 <https://downloads.redpitaya.com/downloads/Clients/labview/Synchronised%20one%20pulse%20signal%20generation%20and%20acquisition.vi>`_
