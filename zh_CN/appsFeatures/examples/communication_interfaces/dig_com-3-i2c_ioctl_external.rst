...
本示例演示如何使用 SCPI 和 API 命令与连接到 Red Pitaya I2C 端口的外部 24LC64 EEPROM 存储器通信。下面的代码先向外部 EEPROM 写入自定义消息，然后将其读回。

.. note::

    以下代码适用于 24LC64 EEPROM 芯片，若用于其他外部 I2C 设备则需要修改。24LC64 EEPROM 硬件地址的低三位由 PCB 上的引脚硬件定义。此外，传输的前两个字节对应 EEPROM 中的目标地址。
    有关正确的寻址方式和通信协议，请参阅 I2C 设备的数据手册。
...
.. note::
    ...
    Python 函数可通过 |redpitaya_scpi| 上提供的最新版 redpitaya_scpi.py 文档访问。
    这些函数优化了使用体验，因为它们会以最佳顺序组合 SCPI 命令。不使用这些函数时，代码仍应以大致相同的速度运行。

有关函数的更多信息，请参阅 redpitaya_scpi.py 代码。
