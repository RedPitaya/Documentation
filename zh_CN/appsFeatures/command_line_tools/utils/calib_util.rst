...
要使用 *calib* 工具正确校准 Red Pitaya，需要了解正在使用的校准格式。共有 4 种不同类型的校准：

* **旧校准格式**。前四种用于 1.04 OS 版本，并取决于 Red Pitaya 板卡型号。
    ...
    可以在此处找到 :rp-github:`校准规格 <RedPitaya/blob/master/rp-api/api-hw-calib/src/calib_structs.h>`。

* **新通用校准格式**，用于 OS 2.00 及更高版本：

    5. 通用校准参数版本 5 - 在 API 中执行校准。
    6. 通用校准参数版本 6 - 在 FPGA 中执行校准。

校准 API 允许使用所有类型的结构，但旧格式只能在向后兼容模式下工作（因此迁移到 OS 2.0 版本时不会出现问题）。
...
下面是使用 *calib* 工具和新通用校准格式的几个示例。

.. note::

    要从旧格式转换为新格式，请从新格式文件加载校准值，或执行一种校准重置操作。
...
校准应用允许用户备份校准参数（出厂校准或用户设置），并从之前保存的备份文件恢复校准。这样可以方便地在 OS 更新期间保留校准数据，或在板卡之间传输设置。

*calib* 工具通过 **-b** （二进制模式）和 **-s** （备份信息）选项，在命令行提供相同功能。
...
.. note::
    ...
    ``-s`` 选项也可以与 ``-v`` 结合，以显示原始校准值（``calib -s -v backup.bin``）。
...
Red Pitaya GitHub 仓库包含 :rp-github:`calib 工具的源代码 <RedPitaya/tree/master/tools/calib>`。
