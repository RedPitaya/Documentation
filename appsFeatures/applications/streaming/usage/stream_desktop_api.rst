...
The board list displays all detected Red Pitaya boards on the same local network running the Streaming Application. Red Pitaya boards that do not have the streaming server running will not be detected. For optimal performance, the boards should be connected to a router.
