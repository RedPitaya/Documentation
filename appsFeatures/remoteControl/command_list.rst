...
| | Example:                                  | |    ``rp_AcqGetDataV(rp_channel_t channel, uint32_t pos, uint32_t* size, float* buffer)``                                               | | PRE, POST trigger configuration returns ``size`` data samples.                       |                    |
| | ``ACQ:SOUR1:DATA:TRig? 3,POST_TRIG`` >    | | Python: ``rp_AcqGetDataRaw(<channel>, <pos>, <size>, <buffer>)``                                                                       | | PRE_POST_TRIG returns data in ``size`` * 2 + 1, where +1 represents the triggering   |                    |
