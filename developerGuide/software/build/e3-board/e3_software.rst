...
E3 add-on module - software
##############################

Also known as the E3 add-on module, it provides secure and robust Red Pitaya boot and shutdown options.
...
.. TODO add picture of the E3 board
...
The E3 add-on module is designed to be used with the Red Pitaya STEMlab 125-14 Pro Gen 2 and STEMlab 125-14 Z7020 Pro Gen 2 boards. It is connected to the Red Pitaya board via the E3 connector. The E3 connector controls the power and booting of the Red Pitaya.
...
The USB to Serial cable can be used to program the E3 board via the UART port, but is currently not supported.
...
Connect the USB to Serial cable between the UART port on the E3 board and the computer. Make sure that the TX and RX pins are connected correctly.
...
6. Open the Arduino sketch for the E3 board. You can find the sketch and the latest version of the E3 board firmware in the .. !PLACEHOLDER!#.
...
#. **Redefine the default weak pinmap declarations.** The pin declarations are defined as "weak" in the *PeripheralPins.h* library. We must redefine them in the arduino sketch as not all the interface pins are used or available on the microcontroller. Add the following lines at the beginning of the arduino sketch:
   ...
   .. code-block:: c
    ...
    .. note::
        ...
        Pin redefinitions are cruicial for proper operation of the E3 add-on module. If the pins are not redefined, the microcontroller will not work as expected.
...
#. **Decleare UART and I2C buses:**
    ...
    I2C1 and UART are currently not supported (there is currently no connector on the E3 board).
...
The E3 board must be connected to a powered Red Pitaya unit during the programming process. The Red Pitaya unit provides power to the E3 board.


Arduin IDE
----------

The program can be uploaded to the E3 board with Arduino IDE in three different ways:
...
1. **ST-Link/V2 programmer (SWD)**
    ...
    * Connect the ST-Link/V2 programmer to the computer and the E3 board.
    * Select *STM32CubeProgrammer (SWD)* as the upload method in the Arduino IDE.
    * Click the upload button in the Arduino IDE.

2. **USB to micro USB cable (DFU)**
    
    * Connect the USB to micro USB cable between the computer and the E3 board.
...
3. **USB to Serial cable (UART) (currently not supported)**
    ...
    * Connect the USB to Serial cable between the computer and the E3 board.
    * Select *STM32CubeProgrammer (Serial)* as the upload method in the Arduino IDE.
    * Click the upload button in the Arduino IDE.
    * When Arduino IDE reaches the uploading stage, press the reset button on the E3 board. This will put the board in the bootloader mode and the program will be uploaded.


STM32 Cube Programmer
---------------------

Alternatively, you can use the STM32CubeProgrammer software to program the E3 board with a compiled binary version of your program. You can use any of the three methods listed above to connect the E3 board to the computer.

1. Open the STM32CubeProgrammer software.
#. Connect the E3 board to the computer.
...
#. Click the "Connect" button. The E3 board should be automatically detected by the STM32CubeProgrammer.

    .. figure:: img/STM32_cube_connected.png
        :alt: STM32CubeProgrammer connected to the E3 board
        :align: center
        :width: 1000
...
#. Click the "Start Programming" button to upload the program to the E3 board.

    .. figure:: img/STM32_cube_program.png
        :alt: STM32CubeProgrammer programming the E3 board
        :align: center
        :width: 1000px
...
E3 board schematics and Pinout
==============================

You can find the E3 board schematics and pinout in the `hardware section <E3_HW>`_.
...
The E3 board firmware is written in Arduino IDE C++ and is available in the `Red Pitaya GitHub repository <https://github.com/RedPitaya/RedPitaya-Examples>`_. (**Link not yet available**)
...
For information on hardware specifications such as pinout of the E3 board, please refer to the `hardware section <E3_HW>`_.
