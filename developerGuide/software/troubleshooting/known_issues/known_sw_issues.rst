...
This issue will be resolved in the 3.00 OS update by updating the Linux kernel driver and supporting new Wi-Fi dongles.
...
JupyterLab Matplotlib library not up to date
---------------------------------------------

**Fixed on OS version 2.07-48 or higher**


I2C crash after multiple read/writes
----------------------------------------

**Fixed on OS version 2.07-48 or higher**

When using the SCPI or C/Python API commands to continuously read/write to an external I2C device, the SCPI server hangs after a few hundred operations.
...
The problem was fixed by fixing the I2C driver to properly close the device after each operation.
...
The :ref:`Streaming desktop application <stream_desktop_app>` and :ref:`command line client <stream_command_client>` may report an EOL error within two minuts of starting the desktop application on OS 2.05-37 and OS 2.07-43. 
This also affects the command line client, where specifying the stream time (-t) parameter will cause the same error.

The problem was related to Streaming application (Data stream control) sending updates less than once per minute, which caused the browser to disconnect and reload the application.
...
**Fixed on OS versions 2.07-48 and higher**
...
**For older OS versions, please follow the instructions below:**

The simplest solution on older OS versions is to add a SWAP space to the Red Pitaya's SD card (about 1 GB should be sufficient).
...
**Fixed on OS versions 2.00-23 and higher**
